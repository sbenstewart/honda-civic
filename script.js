...
class Scene {
  constructor(model) {
    ...
    this.light = new THREE.PointLight(0xffffff, 0.75); this.light.position.z = 150; this.light.position.x = 70; this.light.position.y = 50; this.scene.add(this.light); this.softLight = new THREE.AmbientLight(0xffffff, 2.5); this.scene.add(this.softLight); // group
  ...
  function onModelLoaded() {
    object.traverse(function (child) {
      let mat = new THREE.MeshPhongMaterial({ color: 0x171511, specular: 0xD0CBC7, shininess: 5, flatShading: false });
      child.material = mat;
    });
  ...
  loader.load('./model.obj', function (obj) { object = obj; });
}
...
function setupAnimation(model) {
  ...
  gsap.set(plane.position, { x:60, y: -32, z: -60 });
  ...
  tl.to(plane.rotation, { x: 0, y: -tau * 0.5, z: 0, ease: 'power3.inOut' }, delay);
  tl.to(plane.position, { x: 0, y: 0, z: 0, ease: 'power1.inOut' }, delay);

  delay += sectionDuration;

  tl.to(plane.rotation, { x: 0, y: 0, z: 0, ease: 'power3.inOut' }, delay);
  tl.to(plane.position, { x: 40, y: 0, z: 60, ease: 'power2.inOut' }, delay);

  delay += sectionDuration;

  tl.to(plane.rotation, { x: +tau * 0.2, y: +tau * 0.2, z: +tau * 0.01, ease: 'power3.inOut' }, delay);
  ...
  tl.to(plane.rotation, { x: tau * 0.15, y: tau * .35, z: -tau * 0, ease: 'power1.in' }, delay);
  tl.to(plane.position, { z: -150, x: 0, y: 0, ease: 'power1.inOut' }, delay);

  delay += sectionDuration;

  tl.to(plane.rotation, { duration: sectionDuration, x: -tau * 0.025, y: tau * 0.5, z: -tau * 0.075, ease: 'none' }, delay);
  tl.to(plane.position, { duration: sectionDuration, x: 40, y: 0, z: 220, ease: 'power1.in' }, delay);

  tl.to(scene.light.position, { duration: sectionDuration, x: 0, y: 0, z: 300 }, delay);
}
